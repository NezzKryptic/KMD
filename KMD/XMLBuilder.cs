...
namespace KMD
{
    class XMLBuilder
    {
        public static void BuildXml()
        {
            ...
            {
                Console.WriteLine("Input: " + song + Environment.NewLine);

                YoutubeSearch.FindVideos(song);

                for (int label_index = 0; label_index < Variables.current_titles.Count; label_index++)
                {
                    Console.WriteLine(label_index + ": " + Variables.current_titles[label_index]);
                }
                ...
                xmlWriter.WriteAttributeString("href", Variables.current_hrefs[download_choice]);
